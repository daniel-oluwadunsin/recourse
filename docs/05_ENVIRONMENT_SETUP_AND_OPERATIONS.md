...
- authenticated/private raw assets;
- server-generated signed upload parameters;
- short-lived signed download URLs;
- invalidation on deletion;
- separate staging/production Cloudinary environments or isolated prefixes;
- no API secret in browser or client upload payloads.
...
CLOUDINARY_UPLOAD_FOLDER=recourse
CLOUDINARY_UPLOAD_TTL_SECONDS=900
CLOUDINARY_DOWNLOAD_TTL_SECONDS=300
```

The API uses `resource_type=raw` and `type=authenticated` for original evidence. The
client receives a signed Cloudinary upload intent; the API verifies object metadata,
hashes the object, validates its content signature, and only then marks the evidence
uploaded. Run `pnpm cloudinary:check` with `LIVE_CLOUDINARY_CHECK=true` before staging
acceptance.

## 2.9 Upload limits

```bash
UPLOAD_MAX_BYTES_PDF=26214400
UPLOAD_MAX_BYTES_DOCX=15728640
UPLOAD_MAX_BYTES_EMAIL=26214400
UPLOAD_MAX_BYTES_TEXT=2097152
UPLOAD_MAX_BYTES_IMAGE=15728640
...
## 8.1 Cloudinary access policy

Use authenticated delivery for evidence assets. Keep the API secret server-side and
do not expose Cloudinary Admin API calls to the browser. Separate staging and
production cloud names or prefixes, and enable deletion invalidation.

The Cloudinary API key should be limited to the Cloudinary account/environment used
by the service. Upload signatures are generated by the API with a server-only secret;
clients receive only the signed fields for one opaque object key.

## 8.2 Direct upload policy

The web application posts bytes directly to Cloudinary's raw upload endpoint. The API
does not proxy large uploads. Upload intent and completion endpoints remain owner
scoped, and completion performs provider metadata, size, hash, and magic-byte checks.
...
- [ ] Cloudinary authenticated delivery, environment isolation, upload limits, and lifecycle/deletion behavior;
